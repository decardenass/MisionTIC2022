def AutoPartes(ventas):
    ...
    for i in range(0,len(ventas)):
        if ventas[i][0] in producto:
            producto[ventas[i][0]].append(ventas[i][1:])
        else:
            producto[ventas[i][0]] = [ventas[i][1:]]
    return producto
...
def consultaRegistro(ventas, idProducto):
    ...
    if idProducto in ventas:
        for i in range(len(ventas[idProducto])):
            mensaje = mensaje + "Producto consultado : {idProducto}  Descripción  {dProducto}  #Parte  {pnProducto}  Cantidad vendida  {cvProducto}  Stock  {sProducto}  Comprador {nComprador}  Documento  {cComprador}  Fecha Venta  {fVenta}\n".format(idProducto=idProducto, dProducto=ventas[idProducto][i][0], pnProducto=ventas[idProducto][i][1], cvProducto=ventas[idProducto][i][2], sProducto=ventas[idProducto][i][3], nComprador=ventas[idProducto][i][4], cComprador=ventas[idProducto][i][5], fVenta=ventas[idProducto][i][6])
    else:
